# Baza wiedzy — indeks główny

Baza wiedzy inwestycyjnej budowana iteracyjnie przez agenta AI. Każdy temat powstał w odpowiedzi na konkretne pytanie badawcze.

## Tematy

| Katalog | Opis | Artykułów |
|---------|------|-----------|
| [europejskie-spolki-zbrojeniowe/](europejskie-spolki-zbrojeniowe/) | Główni europejscy producenci uzbrojenia i systemów obronnych | 5 |
| [uran-i-energia-jadrowa/](uran-i-energia-jadrowa/) | ETF-y uranowe, firmy budujące reaktory, korelacje cenowe | 8 |
| [dostepnosc-brokerzy/](dostepnosc-brokerzy/) | Dostępność instrumentów na platformach XTB, Saxo, BOŚ | 3 |

## Statystyki

- **Artykułów:** 16
- **Katalogów:** 7
- **Ostatnia aktualizacja:** 2026-05-26

## Powiązania między tematami

- Spółki z `europejskie-spolki-zbrojeniowe/` → dostępność w `dostepnosc-brokerzy/`
- ETF-y z `uran-i-energia-jadrowa/etf-uranowe/` → skład w `uran-i-energia-jadrowa/firmy-reaktory-jadrowe/`
- Firmy z obu sektorów → dostępność w `dostepnosc-brokerzy/`

---
*Materiał informacyjny — nie stanowi rekomendacji inwestycyjnej.*
